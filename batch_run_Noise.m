...
noise_sigma_All = logspace(-5,-1,11);
...
for iNoise = 1:numel(noise_sigma_All)
    ...
    parfor iGraph = 1:Num_graphs
        ...
        error_MT_All(iNoise,iGraph) = Lerror_MT;

        Lerror_LS = 100*norm(Lhat_LS-L,"fro")/norm(L,"fro");
        error_LS_All(iNoise,iGraph) = Lerror_LS;

        Lerror_LS2 = 100*norm(Lhat_LS2-L,"fro")/norm(L,"fro");
        error_LS2_All(iNoise,iGraph) = Lerror_LS2;
...
figure; 
plot(noise_sigma_All,mean(error_MT_All,2),'-ro'); hold on;
plot(noise_sigma_All,mean(error_LS_All,2),'-bo');
plot(noise_sigma_All,mean(error_LS2_All,2),'-ko');
legend('MT','LS','LS2');
grid on;

figure; 
loglog(noise_sigma_All,mean(error_MT_All,2),'-ro'); hold on;
loglog(noise_sigma_All,mean(error_LS_All,2),'-bo');
loglog(noise_sigma_All,mean(error_LS2_All,2),'-ko');
legend('MT','LS','LS2');
grid on;

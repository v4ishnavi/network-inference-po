...
Num_graphs = 20;
graph_node_num_set = 4:2:12;  % smaller graphs for SI
N_process = 2;
...
for iN = 1:numel(graph_node_num_set)
    ...
    disp(['Graph size ', num2str(iN), '/', num2str(numel(graph_node_num_set)), ...
          ' (N=', num2str(N), ')']);
...
figure; 
plot(graph_node_num_set, mean(error_MT_All,2), '-ro', 'linewidth', 1, 'MarkerSize', 8); 
hold on;
plot(graph_node_num_set, mean(error_LS_All,2), '-bs', 'linewidth', 1, 'MarkerSize', 8);
plot(graph_node_num_set, mean(error_LS2_All,2), '-k^', 'linewidth', 1, 'MarkerSize', 8);
legend('MT','LS','LS2 (Symmetric)');
grid on; box on;
xlabel('Number of nodes');
ylabel('Average relative error %');
title('SI Dynamics: Error vs Graph Size');

figure; 
semilogy(graph_node_num_set, mean(error_MT_All,2), '-ro', 'linewidth', 1, 'MarkerSize', 8); 
hold on;
semilogy(graph_node_num_set, mean(error_LS_All,2), '-bs', 'linewidth', 1, 'MarkerSize', 8);
semilogy(graph_node_num_set, mean(error_LS2_All,2), '-k^', 'linewidth', 1, 'MarkerSize', 8);
legend('MT','LS','LS2 (Symmetric)');
grid on; box on;
xlabel('Number of nodes');
ylabel('Average relative error %');
title('SI Dynamics: Error vs Graph Size (Semilogy)');
fontsize(scale=1.5);
